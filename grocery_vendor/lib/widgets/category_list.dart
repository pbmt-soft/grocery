                    ...
                    snapshot.data.docs.map((DocumentSnapshot document){
                      return ListTile(
                        leading: CircleAvatar(
                          backgroundImage: NetworkImage((document.data() as dynamic)['image']),
                        ),
                        title: Text((document.data() as dynamic)['name']),
                        onTap: (){
                         _provider.selectCategory((document.data() as dynamic)['name'],(document.data() as dynamic)['image']);
                         Navigator.pop(context);
                        },
                      );
                    }).toList(),
      ...
      child: Column(
        children: [
          ...
          FutureBuilder<DocumentSnapshot>(
              future: _services.category.doc(_provider.selectedCat).get(),
              builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                if(snapshot.hasError){
                  return Text('Something went wrong..');
                }
                if(snapshot.connectionState==ConnectionState.done){
                  Map<String, dynamic>data=snapshot.data.data() as dynamic;
                        ...
                        Padding(
                          ...
                          child: Container(
                            child: Row(
                              children: [
                                Text('Main Category : '),
                                FittedBox(child: Text(_provider.selectedCat,style: TextStyle(fontWeight: FontWeight.bold),))
                              ],
                            ),
                          ),
                        ),
